...
```
用户输入 → 意图分类（规则→上下文→LLM 三层）
         → 按 WorkMode 加载工具集（Chat 14 / Plan 21 / Edit 全部）
         → 动态上下文注入（意图标签 + 全局记忆 + repo map + 技能列表）
         → Agent 自主循环（思考→工具调用→流式输出→观察→循环）
         → finalize（token 统计 + 记忆 Agent + 会话标题）
```
