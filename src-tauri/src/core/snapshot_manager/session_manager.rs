...
impl SessionManager {
    ...
    pub async fn new(session_id: &str) -> Result<Self, String> {
        let base_dir = crate::core::data_paths::session_paths(session_id).snapshots_dir();
        std::fs::create_dir_all(&base_dir).map_err(|e| format!("创建快照目录失败: {}", e))?;

        let journal_path = base_dir.join("journal.log");
        let journal = Journal::open(&journal_path).map_err(|e| format!("打开日志失败: {}", e))?;
    ...
    pub async fn create_snapshot(
        ...
        if is_checkpoint {
            ...
            if !workspace.files.is_empty() {
                let content_store = crate::core::data_paths::session_paths(&self.session_id)
                    .snapshots_dir()
                    .join("content");
