...
//!
//! 动态上下文的块顺序（见 `build_dynamic_context()`）：
//! `<context_snapshot>` → `<mode_rules>` → `<intent>` → 能力边界 → `<project_index>` → `<user_profile>`。
//! 注意「能力边界」块不带 XML 标签，它由 `Capabilities::context_block()` 直接生成。
//!
...
/// 构建随用户消息一起落库的动态上下文。
///
/// 只放「每轮才有效、且会随会话变化」的东西：意图标签、能力边界、项目结构、用户画像。
///
