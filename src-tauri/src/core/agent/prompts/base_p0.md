...
- ⚠️ 延迟加载工具（可用工具列表中「延迟工具」分类）：不能直接调用，必须先用 GetToolCatalog 获取可用资源目录，再通过 SearchTools 搜索获取参数定义，最后用 RunDeferredTool(name="工具名", args={...}) 代理执行。技能通过 LoadSkill 直接加载。直接调用延迟工具名将返回未知工具错误
