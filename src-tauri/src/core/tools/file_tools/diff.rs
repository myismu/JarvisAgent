...
//!
//! 将旧文本与新文本转换为 snapshot_engine 使用的 `TextDiff`，保留上下文、插入和删除行，供文件编辑/写入后的快照记录复用。
//!
//! ## Key Exports
//! - `compute_diff()`: 计算两个文本之间的结构化 diff
//!
//! ## Dependencies
//! - Internal: `crate::core::snapshot_engine::patch`
//! - External: `similar`
...
pub(super) fn compute_diff(
    old_text: &str,
    new_text: &str,
) -> crate::core::snapshot_engine::patch::TextDiff {
